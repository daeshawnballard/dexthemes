---
title: DexThemes Harness Integration Status
description: See which coding harnesses have current loaded DexThemes MCP, theme mutation, and exact restore proof—and why incomplete adapters stay out of normal selectors.
...
- **Cursor — 100/100:** Cursor loaded the restricted six-tool plugin inventory and completed a real call; its local theme extension visibly applied and restored the prior theme. Marketplace publication is not implied.
- **T3 Code — 100/100:** T3 Code Nightly loaded six tools, completed a real search, visibly imported a theme, and restored the previous appearance.
...
## Devin is experimental and status-only

Devin is not part of the authoritative 12-harness product roster and does not appear in normal menus. Its connector package and restricted remote MCP service have source and service proof, but no loaded Devin CLI/account proof was performed. A future loaded inventory plus real call could earn the MCP half. No supported Devin appearance mutation seam is known, so the current experiment has a 50-point ceiling and must not be presented as a theme integration.
...
Incomplete adapters stay off normal selectors because discovery, source quality, or a partial score can otherwise look like an Apply promise. They remain readable on the [integration status and support page](/support.html), with their exact blocker and next action, while verified hosts keep the normal product path clear. Release, push, deployment, publication, package publishing, marketplace submission, credential changes, terms acceptance, and user-profile installation for this integrated candidate were not performed.
