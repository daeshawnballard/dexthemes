...
The hardest design constraint was making public discovery frictionless while keeping account writes safe. The plugin therefore exposes public read/compute tools without auth, triggers GitHub OAuth only for personal data or publication, and derives identity solely from the verified token. In the first-party experience, publication follows an exact visual review and a user-pressed Publish button. The current terminal authorization is OAuth `themes:write`; its short-lived confirmation token binds the exact reviewed payload and OAuth session but does not independently attest that a human clicked the button, and app visibility is presentation metadata rather than authorization.
