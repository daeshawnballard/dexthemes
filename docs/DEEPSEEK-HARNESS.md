...
The current npm release is `0.6.2`. The source tree prepares an unreleased `0.6.3` receipt fix; it is not a registry artifact until separately reviewed and published. Upgrade a published version with `plugin ... add @dexthemes/deepseek-harness-plugin@<published-version>`, inspect it with `plugin ... why`, and remove it with `plugin ... remove @dexthemes/deepseek-harness-plugin`. Revert/forget the saved theme and Disconnect before removal.
...
Production auth was repeated with the public npm `0.6.2` bundle on 2026-08-15. Fresh Device Flow connected; the account panel showed six published themes and eight achievements; connected Apply and Revert worked; remount preserved the in-memory connection; and Disconnect returned the UI to anonymous. **Deep Current** was already present, so this did not prove that the fresh connection newly awarded **Harnessed** / **Deep Current**. Two connected Applies produced no observable `/plugin/deepseek-harness/use` request, so `0.6.2` client-reported Connected Apps activity remains unproven. The unreleased `0.6.3` source responds by routing both Apply surfaces through an explicit coordinator with visible acknowledgement and idempotent retry; that fix still requires a reviewed package build and a fresh loaded production-auth receipt. See the [production-auth receipt](DEEPSEEK-HARNESS-062-PRODUCTION-AUTH-RECEIPT.md).
...
| Account features | Existing OAuth bearer identity, stats, unlocks, and protected rewards | Optional in-memory DexThemes Connect session plus explicit post-restart reconnect; verified Device Flow grants `Harnessed` → `Deep Current`, while `/plugin/deepseek-harness/use` records separately scoped client-reported activity | `0.6.2` removes client Apply as an entitlement authority but did not emit an observable production receipt; unreleased `0.6.3` adds explicit coordination and idempotent retry; anonymous use never grants it |
...
The earlier registry-installed `0.6.0` package delivered the bounded Apply/Revert schema and received Statsig HTTP 202 receipts. The registry-installed `0.6.2` gate proved the current UI/runtime lifecycle but intentionally blocked Statsig before navigation, so production delivery of the additive action/outcome, restore, and capability fields remains unproven. Separately, two production connected Applies emitted no observable `/plugin/deepseek-harness/use` request; the unreleased `0.6.3` repair remains source/build proof until a reviewed package produces an acknowledged request. No prompt, workspace, URL, account, credential, or palette data is permitted in either path.
