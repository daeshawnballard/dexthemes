...
6. A successful connected theme apply calls the separately scoped `/plugin/deepseek-harness/use` route with a random UUID receipt. Convex dedupes a repeated receipt and records the last-used time and count as explicitly client-reported activity; this claim cannot grant a protected reward. No theme, palette, prompt, workspace, or token enters that record. Disconnect marks the integration inactive only after Convex acknowledges session revocation; failures remain connected and retryable, and expiry is the final credential fallback.
...
The current npm release is `0.6.2`. Upgrade a published version with `plugin ... add @dexthemes/deepseek-harness-plugin@<published-version>`, inspect it with `plugin ... why`, and remove it with `plugin ... remove @dexthemes/deepseek-harness-plugin`. Revert/forget the saved theme and Disconnect before removal.
...
Refresh the Harness browser tab after adding the package, then open **Settings → Plugins → DexThemes**. Version `0.6.2` was installed from the public npm registry into a fresh isolated Harness profile with the lockfile resolving the registry's published SHA-512 integrity and no local/workspace source. The bundle served by real Harness `0.1.0-rc.5` matched the installed package byte-for-byte. The loaded plugin exposed 111 bundled and connected public/community themes, rendered paired Alibaba previews, applied through Harness's guarded theme service, restored the saved intent after a full process restart when the DexThemes surface remounted, reverted to the native Harness default, and retained 100 bundled themes during a controlled catalog 503. See the [loaded-runtime receipt](DEEPSEEK-HARNESS-062-RUNTIME-RECEIPT.md).
...
The production **DexThemes Connect** Device Flow was completed from the earlier clean `0.6.0` installation. The connected Settings UI returned the account's six published themes and seven existing achievements without exposing a GitHub token. That release then granted **Deep Current** after a connected Apply; `0.6.2` supersedes that client-authoritative condition by granting the reward during server-verified Device Flow completion and treating later Apply activity only as client-reported telemetry. The `0.6.2` runtime gate did not repeat production Device Flow, so immediate production `Harnessed` / `Deep Current` appearance remains unproven for the new grant boundary.
...
The `0.6.2` contract targets the same DeepSeek Harness CLI `0.1.0-rc.5` integration boundary verified for `0.6.1` at commit `47f943859bef60e4160492346772ded9b24f765a`. Required client contracts are the Module Loader, `settings.plugins.tab`, persisted `defineStore`, optional Cordis service injection, and `ThemeRuntime.overrideTokens(source, pairedTokens) → disposer`. No broader Harness semver range is claimed. Harness version is not added to analytics unless the client exposes an authoritative value.
...
| Revert and restart | None | Retained installed-plugin disposer, persisted bounded theme intent, `cordis_stop` for a dynamic Package | Registry-loaded `0.6.2` proves full process restart restoration on DexThemes-surface remount plus Revert to Harness defaults |
...
| Account features | Existing OAuth bearer identity, stats, unlocks, and protected rewards | Optional in-memory DexThemes Connect session plus explicit post-restart reconnect; verified Device Flow grants `Harnessed` → `Deep Current`, while `/plugin/deepseek-harness/use` records separately scoped client-reported activity | `0.6.2` removes client Apply as an entitlement authority; anonymous use never grants it |
...
| Analytics storage | Existing Statsig project and public client-key config | Package-owned Statsig client with allowlisted platform/source/version/theme/variant/action/outcome metadata and lifecycle disposal | The `0.6.2` runtime gate intentionally blocked Statsig; production delivery of the additive fields remains unproven |
...
The earlier registry-installed `0.6.0` package delivered the bounded Apply/Revert schema and received Statsig HTTP 202 receipts. The registry-installed `0.6.2` gate proved the current UI/runtime lifecycle but intentionally blocked Statsig before navigation. Production delivery of the additive action/outcome, restore, capability, and separately scoped client-reported use fields therefore remains unproven until a production receipt is captured. No prompt, workspace, URL, account, credential, or palette data is permitted in either schema.
