...
Production auth was repeated with the public npm `0.6.2` bundle on 2026-08-15. Fresh Device Flow connected; the account panel showed six published themes and eight achievements; connected Apply and Revert worked; remount preserved the in-memory connection; and Disconnect returned the UI to anonymous. **Deep Current** was already present, so this did not prove that the fresh connection newly awarded **Harnessed** / **Deep Current**. Two connected Applies produced no observable `/plugin/deepseek-harness/use` request, so `0.6.2` client-reported Connected Apps activity remains unproven. The reviewed `0.6.3` source responds by routing both Apply surfaces through an explicit coordinator with visible acknowledgement and idempotent retry; that fix still requires registry publication and a fresh loaded production-auth receipt. See the [production-auth receipt](DEEPSEEK-HARNESS-062-PRODUCTION-AUTH-RECEIPT.md).
...
| Account features | Existing OAuth bearer identity, stats, unlocks, and protected rewards | Optional in-memory DexThemes Connect session plus explicit post-restart reconnect; verified Device Flow grants `Harnessed` → `Deep Current`, while `/plugin/deepseek-harness/use` records separately scoped client-reported activity | `0.6.2` removes client Apply as an entitlement authority but did not emit an observable production receipt; reviewed `0.6.3` adds explicit coordination and idempotent retry but still needs registry-loaded proof; anonymous use never grants it |
