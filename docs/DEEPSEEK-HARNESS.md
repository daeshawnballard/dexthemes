...
The production **DexThemes Connect** Device Flow was also completed from the clean `0.6.0` installation. The connected Settings UI returned the account's six published themes and seven existing achievements without exposing a GitHub token. That release then granted **Deep Current** after a connected Apply; `0.6.2` supersedes that client-authoritative condition by granting the reward during server-verified Device Flow completion and treating later Apply activity only as client-reported telemetry. Revert restored Harness default, and anonymous browsing and applying remained available.
