...
- `POST /plugin/deepseek-harness/use` requires the dedicated `harness:use` scope and accepts only a UUID receipt plus an optional normalized plugin version—never an action, user, platform, prompt, workspace, credential, theme, or palette payload. It updates replay-deduped, explicitly client-reported Connected Apps activity and cannot grant a protected reward.
