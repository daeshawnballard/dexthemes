...
The public-registry `0.6.4` receipt proves fresh exact-package installation, loaded bundle identity, Apply, full process restart with restoration on DexThemes-surface remount, Revert, 100-theme offline fallback, fresh Device Flow, complete user-initiated code copy, account data loading, connected activity acknowledgement, bounded Statsig delivery, and Creator-mode chat Apply/Stop. It does not claim that the reconnect newly awarded an achievement already present on the account. See the [0.6.4 release receipt](DEEPSEEK-HARNESS-064-RUNTIME-RECEIPT.md), [the original 0.6.2 runtime receipt](DEEPSEEK-HARNESS-062-RUNTIME-RECEIPT.md), and [the superseded 0.6.2 production-auth receipt](DEEPSEEK-HARNESS-062-PRODUCTION-AUTH-RECEIPT.md).

- [x] Start from a fresh isolated Harness home/profile and install the current published npm package; record the resolved version and integrity separately from source proof
- [x] Open **Settings → Plugins → DexThemes**, connect through an approved test Device Flow, and Apply a paired theme from the exact registry package
- [x] Quit only the isolated Harness process, relaunch the same profile, and reopen DexThemes
- [x] **Expected**: no token, prompt, workspace content, palette, or account identifier is persisted; the validated saved theme restores when its required catalog is available, while account reconnect remains explicit after a later restart
