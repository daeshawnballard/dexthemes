...
test('host export publication binds the parent before an ancestor substitution without writing through it', async () => {
  ...
  const bundle = await buildHostExports({
    ...
    testHooks: {
      afterDirectoryBound: async ({ stageRoot: staged, bundle: receivedBundle }) => {
        stageRoot = staged;
        // This happens after both parent descriptors are acquired, immediately
        // before descriptor-relative activation. A pathname rename here would
        // write through the replacement symlink.
        assert.deepEqual(
          JSON.parse(await readFile(path.join(stageRoot, 'MANIFEST.json'), 'utf8')),
          receivedBundle.manifest,
        );
  ...
  assert.deepEqual(
    JSON.parse(await readFile(new URL('host-exports/MANIFEST.json', movedParent), 'utf8')),
    bundle.manifest,
    'descriptor-bound target parent receives the activation after its pathname is swapped',
  );
});
...
test('host export publication rolls back a moved output when activation fails after backup', async () => {
  ...
  await assert.rejects(
    ...
    /intentional activation failure after descriptor-bound backup/,
  );
